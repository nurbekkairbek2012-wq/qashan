...
/**
 * Второй проход с замечанием от кода.
 *
 * Замечание формулирует НЕ модель и не человек, а клиентская цепочка
 * (core/extraction.js): она проверяет арифметику первого ответа и, если
 * платёж × срок не сходится с суммой, присылает сюда конкретное описание
 * расхождения. Здесь оно просто подклеивается к промпту.
 *
 * Текст замечания не доверяем как есть: это данные, пришедшие в запросе,
 * и они обрезаются по длине, чтобы через них нельзя было подменить инструкции.
 */
const MAX_HINT_LENGTH = 1200;

function buildPrompt(hint: string | null): string {
  if (!hint) return PROMPT;

  return `${PROMPT}

--- ПОВТОРНОЕ ЧТЕНИЕ ---
Это второй проход по тому же изображению. Код проверил твой прошлый ответ
арифметически и нашёл несоответствие:

${hint.slice(0, MAX_HINT_LENGTH)}

Правила прежние: возвращай строки как на экране, не пересчитывай, не подгоняй
числа друг под друга. Если поля нет — null.`;
}
...
Deno.serve(async (req) => {
  ...
  let payload: { image?: string; mimeType?: string; hint?: string };
  try {
  ...
  const { image, mimeType = 'image/png', hint = null } = payload;
      ...
      `https://generativelanguage.googleapis.com/v1beta/models/${MODEL}:generateContent?key=${GEMINI_KEY}`,
      {
        ...
        body: JSON.stringify({
          contents: [
            {
              parts: [
                { text: buildPrompt(typeof hint === 'string' ? hint : null) },
  ...
  return json({
    ...
    model: MODEL,
    pass: hint ? 2 : 1, // видно в отладке, каким проходом получен ответ
  });
});
